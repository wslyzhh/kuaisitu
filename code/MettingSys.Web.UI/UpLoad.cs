...
namespace MettingSys.Web.UI
{
    public class UpLoad
    {
        ...
        /// <param name="ftype">文件类型：1全部人员可查看，2，执行人员不可查看</param>
        /// <returns>上传成功返回JSON字符串</returns>
        public string OrderFileSaveAs(byte[] byteData, string fileName,string oID,string ftype)
        {
            try
            {
                string fileExt = Path.GetExtension(fileName).Trim('.'); //文件扩展名，不含“.”
                ...
                string upLoadPath = sysConfig.filepath + "/" + oID + "/" + ftype + "/"; //本地上传目录相对路径
                string fullUpLoadPath = System.AppDomain.CurrentDomain.BaseDirectory.ToString() + upLoadPath; //本地上传目录的物理路径
                string newFilePath = upLoadPath + newFileName; //本地上传后的路径                
                ...
                {
                    return "{\"status\": 0, \"msg\": \"文件名中不能含有以下特殊字符：" + this.sysConfig.FileNameFilter + "\"}";
                }
                //如果是图片，检查图片是否超出最大尺寸，是则裁剪
                //if (IsImage(fileExt) && (this.sysConfig.imgmaxheight > 0 || this.sysConfig.imgmaxwidth > 0))
                //{
                ...
                //处理完毕，返回JOSN格式的文件信息
                return "{\"status\": 1, \"msg\": \"上传文件成功！\", \"name\": \""
                    + fileName + "\", \"path\": \"" + newFilePath + "\", \"size\": " + Math.Round((decimal)byteData.Length / 1024, 2, MidpointRounding.AwayFromZero)  + ", \"ext\": \"" + fileExt + "\"}";
            }
            catch (Exception err)
            {
                return "{\"status\": 0, \"msg\": \"上传过程中发生意外错误！\"}";
        ...
        /// <param name="ftype">文件类型：1全部人员可查看，2，执行人员不可查看</param>
        /// <returns>上传成功返回JSON字符串</returns>
        public string PayFileSaveAs(byte[] byteData, string fileName, bool isThumbnail, int id, string ftype)
        {
            try
            {
                string fileExt = Path.GetExtension(fileName).Trim('.'); //文件扩展名，不含“.”
                ...
                string upLoadPath = "uploadPay/" + ftype + "/" + id + "/"; //本地上传目录相对路径
                string fullUpLoadPath = System.AppDomain.CurrentDomain.BaseDirectory.ToString() + upLoadPath; //本地上传目录的物理路径
                string newFilePath = upLoadPath + newFileName; //本地上传后的路径                
                string newThumbnailPath = upLoadPath + newThumbnailFileName; //本地上传后的缩略图路径
                byte[] thumbData = null; //缩略图文件流
                ...
                return "{\"status\": 1, \"msg\": \"上传文件成功！\", \"name\": \""
                    + fileName + "\", \"path\": \"" + newFilePath + "\", \"thumb\": \""
                    + newThumbnailPath + "\",\"size\": " + Math.Round((decimal)byteData.Length / 1024, 2, MidpointRounding.AwayFromZero) + ", \"ext\": \"" + fileExt + "\"}";
            }
            catch (Exception err)
            {
                return "{\"status\": 0, \"msg\": \"上传过程中发生意外错误！\"}";
        ...
        {
            string[] list = this.sysConfig.FileNameFilter.Split(',');
