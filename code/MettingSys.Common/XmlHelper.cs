...
namespace MettingSys.Common
{
    public class XmlHelper
    {
        ...
        public static void getNodeContent(string fileUrl, string nodeUrl,string nodeName, out string title, out string content)
        {
            title = ""; content = "";
            try
            {
                XmlDocument doc = new XmlDocument();
                doc.Load(fileUrl);
                XmlNode xn = doc.SelectSingleNode(nodeUrl + "[@name='" + nodeName + "']");
                XmlNodeList xnList = xn.ChildNodes;  //得到该节点的子节点
                title = xnList[0].InnerText;
                content = xnList[1].InnerText;
            }
            catch
            {
