...
function loadMenuTree(_islink) {
    ...
    $.ajax({
        type: "post",
        url: "tools/admin_ajax.ashx?action=get_navigation_list&navtype=oa&time=" + Math.random(),
        dataType: "html",
...
function initMenuTree(islink) {
	...
	navGroupObj.each(function (i) {
        ...
        //ryan 20190414 update
        //var navHtml = $('<a>' + $(this).children("h1").attr("title") + '</a>').appendTo(navObj);
        //var navHtml = $('<li><a><img src="' + $(this).children("h1").children("img").attr("src") + '" title="' + $(this).children("h1").children("img").attr("alt") + '" /><h1>' + $(this).children("h1").attr("title") + '</h1></a></li>').appendTo(navObj);
        var navHtml = $('<li><a><img src="' + $(this).children("h1").children("img").attr("src") + '"/><h1>' + $(this).children("h1").attr("title") + '</h1></a></li>').appendTo(navObj);
        //ryan 20190414 end

		//默认选中第一项 ryan 20190414 update
		if (i == 0) {
			$(this).show();
			//navHtml.addClass("selected");
            navHtml.children("a").addClass("selected");
		}
		//为菜单添加事件 ryan 20190414 update
		navHtml.click(function () {
            navObj.children("li").children("a").removeClass("selected");
		...
		$(this).find("ul").each(function (j) { //遍历所有的UL
			...
			$(this).children("li").each(function () {
                ...
                if (liObj.children("ul").length > 0) {
                    //ryan 20190414 insert
                    liObj.addClass("left-menu-bg"); 
					...
					//如果a有自定义图标则将图标插入，否则使用默认的样式  ryan 20190414 update
					if(typeof(liObj.children("a").attr("icon"))!="undefined"){
						lastIconObj.append('<img src="' + liObj.children("a").attr("icon") + '" />');
					...
					if(typeof(liObj.children("a").attr("href"))!="undefined"){
						...
						liObj.children("a").click(function () {
							...
							if(typeof($(this).attr("navid"))!="undefined"){
                                addCookie("MettingSys_manage_navigation_cookie", $(this).attr("navid"), 240);
							}
						});
					}
				}
