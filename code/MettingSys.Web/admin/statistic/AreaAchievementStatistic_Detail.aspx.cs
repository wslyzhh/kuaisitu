...
{
    public partial class AreaAchievementStatistic_Detail : Web.UI.ManagePage
    {
        ...
        protected string action = "", _page = "",_orderNum="",_cusName="",_content="",_address="", _sMonth = "", _eMonth = "", _status = "", _lockstatus = "", _area = "", _isCust = "", _excel = "", _ordertype = "", _order = "";
        ...
        private void RptBind(string _strWhere, string _orderby)
        {
            ...
            DataTable dt = bll.getAreaAchievementStatisticDeatil(dict, this.pageSize, this.page, _orderby, out this.totalCount, out _tShou, out _tUnIncome, out _tFu, out _tUnCost, out _tCust, out _tTicheng, out _tProfit1, out _tProfit2).Tables[0];
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("AreaAchievementStatistic_Detail_page_size", "DTcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(DTRequest.GetFormString("txtPageNum"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("AreaAchievementStatistic_Detail_page_size", "DTcmsPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            return Utils.CombUrlTxt("AreaAchievementStatistic_Detail.aspx", "page={0}&txtsDate={1}&txteDate={2}&ddlstatus={3}&ddllock={4}&ddlarea={5}&cbIsCust={6}&action={7}&ddlorderType={8}&ddlorder={9}&txtOrderID={10}&txtCusName={11}&txtContent={12}&txtAddress={13}", "__id__", _sMonth, _eMonth, _status, _lockstatus, _area, _isCust, action, _ordertype, _order,_orderNum,_cusName,_content,_address);
        }
        ...
        protected void Excel()
        {
            BLL.statisticBLL bll = new BLL.statisticBLL();
            Dictionary<string, string> dict = getDict();
            DataTable dt = bll.getAreaAchievementStatisticDeatil(dict, this.pageSize, this.page, _ordertype + " " + _order, out this.totalCount, out _tShou, out _tUnIncome, out _tFu, out _tUnCost, out _tCust, out _tTicheng, out _tProfit1, out _tProfit2, false).Tables[0];
            ...
            headRow.CreateCell(5).SetCellValue("区域");
            ...
            if (dt != null)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    row.CreateCell(5).SetCellValue(Utils.ObjectToStr(dt.Rows[i]["p_chnName"])+"("+ dt.Rows[i]["p_ratio"] + "%)");
