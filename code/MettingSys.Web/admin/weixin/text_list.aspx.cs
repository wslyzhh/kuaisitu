...
namespace MettingSys.Web.admin.weixin
{
    public partial class text_list : Web.UI.ManagePage
    {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("weixin_response_text_page_size", "MettingSysPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("weixin_response_text_page_size", "MettingSysPage", _pagesize.ToString(), 14400);
                }
            }
