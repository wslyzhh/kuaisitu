...
namespace MettingSys.Web.admin.finance
{
    public partial class receipt_list : Web.UI.ManagePage
    {
        ...
        protected string _cusName = "", _cid = "", _method = string.Empty, _isconfirm = string.Empty, _sforedate = string.Empty, _eforedate = string.Empty, _sdate = string.Empty, _edate = string.Empty, _num = "", _chk = "", _numdate = "", _self = "",_moneyType="1", _sign = "", _money = "",_type="";
        ...
        protected string CombSqlTxt()
        {
            ...
            if (_self == "1")
            {
                strTemp.Append(" and rp_personNum='" + manager.user_name + "'");
            }
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("Receipt_list_size", "DTcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("Receipt_list_size", "DTcmsPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            ChkAdminLevel("sys_payment_list1", DTEnums.ActionEnum.Delete.ToString()); //检查权限
            BLL.ReceiptPay bll = new BLL.ReceiptPay();
            manager = GetAdminInfo();
            string result = "";
