...
{
    public partial class selfFinance_list : Web.UI.ManagePage
    {
        protected int totalCount; //数据总记录数
        protected int page; //当前页码
        protected int pageSize; //每页大小
        protected string _ordernum = string.Empty, _cusname = string.Empty, _cusid = string.Empty, _status = string.Empty, _type = "", _smonth = "", _emonth = "";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                InitData();
                RptBind(CombSqlTxt(), "fin_adddate desc,fin_id desc");
            }
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("selfFinance_page_size", "DTcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("selfFinance_page_size", "DTcmsPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("selfFinance_list.aspx", "page={0}&ddltype={1}&txtOrder={2}&txtCusName={3}&hCusId={4}&ddlcheck={5}&txtsDate={6}&txteDate={7}", "__id__", _type, _ordernum, _cusname, _cusid, _status, _smonth, _emonth));
        }
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            _emonth = DTRequest.GetString("txteDate");
            BLL.finance bll = new BLL.finance();
            DataTable dt = bll.GetList1(this.pageSize, this.page,CombSqlTxt(), "fin_adddate desc,fin_id desc", out this.totalCount, false).Tables[0];

            HttpContext.Current.Response.Clear();
            HttpContext.Current.Response.AddHeader("Content-Disposition", "attachment; filename=审批未通过应收付.xlsx"); //HttpUtility.UrlEncode(fileName));
            ...
            headRow.CreateCell(1).SetCellValue("收付性质");
            headRow.CreateCell(2).SetCellValue("应收付对象");
            headRow.CreateCell(3).SetCellValue("对账凭证");
            headRow.CreateCell(4).SetCellValue("业务性质/明细");
            headRow.CreateCell(5).SetCellValue("业务日期");
            headRow.CreateCell(6).SetCellValue("业务说明");
            headRow.CreateCell(7).SetCellValue("金额表达式");
            headRow.CreateCell(8).SetCellValue("金额");
            headRow.CreateCell(9).SetCellValue("区域");
            headRow.CreateCell(10).SetCellValue("结账月份");
            headRow.CreateCell(11).SetCellValue("审批状态");
            headRow.CreateCell(12).SetCellValue("添加人");
            ...
            if (dt != null)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    row.CreateCell(1).SetCellValue(Utils.ObjectToStr(dt.Rows[i]["fin_type"])=="True"?"应收":"应付");
                    row.CreateCell(2).SetCellValue(Utils.ObjectToStr(dt.Rows[i]["c_name"]));
                    row.CreateCell(3).SetCellValue(Utils.ObjectToStr(dt.Rows[i]["chk"]));
                    row.CreateCell(4).SetCellValue(Utils.ObjectToStr(dt.Rows[i]["na_name"]) + "/" + Utils.ObjectToStr(dt.Rows[i]["fin_detail"]));
                    row.CreateCell(5).SetCellValue(ConvertHelper.toDate(dt.Rows[i]["fin_sdate"]).Value.ToString("yyyy-MM-dd") + "-" + ConvertHelper.toDate(dt.Rows[i]["fin_edate"]).Value.ToString("yyyy-MM-dd"));
                    row.CreateCell(6).SetCellValue(Utils.ObjectToStr(dt.Rows[i]["fin_illustration"]));
                    row.CreateCell(7).SetCellValue(Utils.ObjectToStr(dt.Rows[i]["fin_expression"]));
                    row.CreateCell(8).SetCellValue(Utils.ObjectToStr(dt.Rows[i]["fin_money"]));
                    row.CreateCell(9).SetCellValue(dt.Rows[i]["fin_area"].ToString());
                    row.CreateCell(10).SetCellValue(Utils.ObjectToStr(dt.Rows[i]["fin_month"]));
                    row.CreateCell(11).SetCellValue(dt.Rows[i]["fin_flag"].ToString() == "0" ? "待审批" : dt.Rows[i]["fin_flag"].ToString() == "1" ? "审批未通过" : "审批通过");
                    row.CreateCell(12).SetCellValue(dt.Rows[i]["fin_personName"].ToString());
