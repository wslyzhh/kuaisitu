...
namespace MettingSys.Web.tools
{
    ...
    public class upload_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                ...
                case "catchimage": //编辑器远程抓取图片
                    ...
                    break;
                case "UpLoadOrderFile":
                    UpLoadOrderFile(context);
                    break;
                case "deleteOrderFile":
                    deleteOrderFile(context);
                    break;
                case "UpLoadPayFile":
                    UpLoadPayFile(context);
                    break;
                case "deletePayFile":
                    deletePayFile(context);
                    break;
                default: //普通上传
                    UpLoadFile(context);
                    break;
        ...
        private void UpLoadFile(HttpContext context)
        {
            ...
            byte[] byteData = System.Text.Encoding.UTF8.GetBytes(DTRequest.GetString("byteFile"));//FileHelper.ConvertStreamToByteBuffer(context.Request.InputStream); //获取文件流
