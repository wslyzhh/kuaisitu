...
namespace MettingSys.Web
{
    public partial class _default : System.Web.UI.Page
    {
        ...
        protected void BtnLogin_Click(object sender, EventArgs e)
        {
            ...
            string userCode = txtValidcode.Text.Trim();

            if (Session[DTKeys.SESSION_CODE] == null)
            {
                msgtip.InnerHtml = "提示：验证码超时，请重新登录";
                return;
            }
            else if (userCode.Equals(""))
            {
                msgtip.InnerHtml = "提示：请输入验证码";
                return;
            }
            else if (userCode.ToLower() != Session[DTKeys.SESSION_CODE].ToString())
            {
                msgtip.InnerHtml = "提示：验证码输入错误";
                return;
            }
            ...
            Utils.WriteCookie("AdminName", "MettingSys", model.user_name);
            Utils.WriteCookie("AdminPwd", "MettingSys", model.password);
