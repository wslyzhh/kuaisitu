...
namespace MettingSys.DAL
{
    ...
    public partial class manager_role
    {
        ...
        public bool Delete(int id)
        {
            ...
            //如果角色已经分配给员工了，则不能删除角色
            string sql = "select count(*) from " + databaseprefix + "manager where role_id=@role_id";
            SqlParameter[] parameters3 = {
                    new SqlParameter("@role_id", SqlDbType.Int,4)};
            parameters3[0].Value = id;
            if (DbHelperSQL.Exists(sql, parameters3))
            {
                return false;
            }
            //删除管理员
            ...
            sq.Append("delete from MS_userRolePemission where urp_roleid=@id");
            SqlParameter[] parameters0 = {
                    new SqlParameter("@id", SqlDbType.Int,4)};
            parameters0[0].Value = id;
            cmd = new CommandInfo(sq.ToString(), parameters0);
            sqllist.Add(cmd);
        ...
        public DataSet GetList(string strWhere)
        {
            ...
            strSql.Append(" order by role_name,id");
