...
namespace MettingSys.DAL
{
    ...
    public partial class businessDetails
    {
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from  MS_Details");
            strSql.Append(" where ");
        ...
        public int Add(Model.businessDetails model)
        {
            ...
            strSql.Append("insert into MS_Details(");
        ...
        public bool Update(Model.businessDetails model)
        {
            ...
            PropertyInfo[] pros = model.GetType().GetProperties();
            List<SqlParameter> paras = new List<SqlParameter>();
            strSql.Append("update MS_Details set ");
            foreach (PropertyInfo pi in pros)
            {
                //如果不是主键则追加sql字符串
                if (!pi.Name.Equals("de_id"))
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from  MS_Details ");
            strSql.Append(" where de_id=@id");
            SqlParameter[] parameters = {
                    new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM MS_Details left join MS_Nature on de_nid=na_id");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        /// 是否存在该记录
        /// </summary>
        public bool Exists(string name,int naid,int id=0)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(0) from MS_Details");
            strSql.Append(" where de_nid=@nid and de_name=@name");
            if (id > 0)
            {
        ...
        /// 将对象转换实体
        /// </summary>
        public Model.businessDetails DataRowToModel(DataRow row)
        {
            Model.businessDetails model = new Model.businessDetails();
            if (row != null)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update MS_Details set " + strValue);
            strSql.Append(" where de_id=" + id);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) from MS_finance where fin_detail =@id");
            SqlParameter[] parameters = {
                    new SqlParameter("@id", SqlDbType.Int,4)
            };
            parameters[0].Value = id;
