...
namespace MettingSys.DAL
{
    ...
    public partial class Order
    {
        ...
        public DataSet GetList(int pageSize, int pageIndex, string strWhere, string filedOrder, out int recordCount,bool type=false,string where="",bool isPage = true,string orderType="",string currentUser ="")
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select *,unMoney=finMoney - rpdMoney,profit=finMoney-finMoney1-isnull(o_financeCust,0) from (select o.*,c.*,co.*,op1.* "+addfield+ ",person2 = (SELECT op_name FROM MS_OrderPerson WHERE op_oid=o_id and op_type=2),person3 = isnull(STUFF((SELECT ',' + op_name+'('+(case when op_dstatus=0 then '待定' else case when op_dstatus=1 then '处理中' else case when op_dstatus=2 then '已完成' else '待定与处理中' end end end)+')' FROM MS_OrderPerson WHERE  op_oid=o_id and op_type=3 FOR XML PATH('')), 1, 1, ''),'无'),place=stuff((select ','+p_name+'('+cast(p_ratio as varchar)+'%)' from MS_OrderPlace where p_oid = o_id for xml path('')),1,1,'') ");
            strSql.Append(" , person4 = isnull(STUFF((SELECT ',' + op_name + '(' + (case when op_dstatus = 0 then '待定' else case when op_dstatus = 1 then '处理中' else case when op_dstatus=2 then '已完成' else '待定与处理中' end end end) + ')' FROM MS_OrderPerson WHERE  op_oid = o_id and op_type = 5 FOR XML PATH('')), 1, 1, ''),'无') ,person6 = isnull(STUFF((SELECT ',' +op_name+'('+cast(op_ratio as varchar)+'%)' FROM MS_OrderPerson WHERE  op_oid=o_id and op_type=6 FOR XML PATH('')), 1, 1, ''),'无'),finMoney=isnull((select sum(isnull(fin_money,0)) fin_money from MS_finance where fin_type=1 and fin_oid=o_id),0),finMoney1=isnull((select sum(isnull(fin_money,0)) fin_money from MS_finance where fin_type=0 and fin_oid=o_id),0),rpdMoney = isnull((select sum(isnull(rpd_money,0)) rpd_money from MS_ReceiptPayDetail left join MS_ReceiptPay on rp_id=rpd_rpid where rpd_type=1 and rp_isConfirm=1 and rpd_oid=o_id),0) FROM MS_Order o left join ms_customer c on o_cid=c_id left join ms_contacts co on o_coid=co_id left join ms_orderperson op1 on o_id=op_oid and op_type=1 " + addTable + ") t");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
